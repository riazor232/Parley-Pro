      ...
      <header className="bg-[#1e3932] dark:bg-[#070e0a] text-white border-b border-green-900/10 dark:border-zinc-900/60 sticky top-0 z-50 shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div>
            ...
            <p className="text-xs text-green-200/70 mt-0.5">Plataforma Predictiva de Inteligencia Deportiva</p>
          </div>
          
          <div className="flex items-center gap-4">
            <div className={`flex items-center gap-2.5 border px-3.5 py-1.5 rounded-full ${serverStatus === 'API Real' ? 'bg-green-950/40 border-green-900/30' : serverStatus === 'Cargando...' ? 'bg-gray-950/40 border-gray-900/30' : 'bg-yellow-950/40 border-yellow-900/30'}`}>
              <span className="relative flex h-2.5 w-2.5">
                ...
                <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${serverStatus === 'API Real' ? 'bg-green-500' : serverStatus === 'Cargando...' ? 'bg-gray-500' : 'bg-yellow-500'}`}></span>
              </span>
              <span className={`text-xs font-semibold ${serverStatus === 'API Real' ? 'text-green-300' : serverStatus === 'Cargando...' ? 'text-gray-300' : 'text-yellow-300'}`}>
        ...
        {error ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
              ...
              {/* Right Column: BetSlip & SavedParleys (1 col) */}
              <div className="space-y-6">
                <BetSlip
